...
sed -E 's|([0-9]{1})/([0-9]{1})/([0-9]{4})|0\1/0\2/\3|g' data.csv | sed -E 's|([0-9]{2})/([0-9]{2})/([0-9]{2};)|\1/\2/20\3|g' | sed -E 's|([0-9]{2})/([0-9]{2})/([0-9]{4})|\3-\2-\1|g' | sed 's/;;/;\\N;/g; s/;N/;\\N/g'  | sed 's/\\n/\\N/g' | sed 's/n/\\N/g' |  sed  's/,/./g' |  sed  's/;/,/g'  | sed 's/,\s*$/,\\N/' | sed -e  's/[a-z]/\u&/g'